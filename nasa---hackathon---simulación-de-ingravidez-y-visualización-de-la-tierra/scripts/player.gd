...
func _ready():
	...
	rotation_degrees.y = cupola.rotation_degrees.y - 180
...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("cancel_movement"):
		...
		var rect = UI.get_node("BlackRect")
		
		var tween = create_tween()
		tween.set_ease(Tween.EASE_OUT)
		tween.set_trans(Tween.TRANS_CUBIC)
		tween.tween_property(rect, "color:a", 1.0, 1)
		...
		tween.set_ease(Tween.EASE_OUT)
		tween.set_trans(Tween.TRANS_CUBIC)
		tween.tween_property(rect, "color:a", 0, 1)
		tween.parallel().tween_property($Camera, "fov", 75, 1)
